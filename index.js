...
function genPlateSetup(data, cb) {
  ...
  loadTemplate(path.join(__dirname, 'template/apldbio/sds/plate_setup.xml'), function(err, doc) {
    ...
    node.innerHTML = sanitize(data.name) || "Unnamed plate";

    node = doc.querySelector('Plate > Description');
    node.innerHTML = sanitize(data.description) || "Generated by renegade-lims";
...
function genExperiment(dirpath, filename, data, cb) {
  loadTemplate(path.join(__dirname, 'template/apldbio/sds/experiment.xml'), function(err, doc) {
    ...
    node.innerHTML = sanitize(data.name) || "Unnamed experiment";

    node = doc.querySelector('Experiment > Operator');
    node.innerHTML = sanitize(data.operator) || "Unknown operator";
